...
for i, review_item in enumerate(review_items, start=1):
    ...
    review_data['review_stars'] = review_stars_dom.text if review_stars_dom else 'No Stars'

    # 작성자
    review_writer_dom = soup_review_item.select_one('._2FmJXrTVEX strong._3QDEeS6NLn')
    review_data['review_writer'] = review_writer_dom.text if review_writer_dom else 'No Writer'

    # 작성일자
    review_date_dom = soup_review_item.select_one('._2FmJXrTVEX span._3QDEeS6NLn')
    review_date_str = review_date_dom.text if review_date_dom else 'No Date'
    review_data['review_date'] = review_date_dom.text if review_date_dom else 'No Content'

    # 리뷰내용
    review_content_dom = soup_review_item.select_one('._19SE1Dnqkf span._3QDEeS6NLn')
    review_data['review_content'] = review_content_dom.text if review_content_dom else 'No Content'

    # 리뷰 이미지
    review_img_dom = soup_review_item.select('img')
    review_data['review_img'] = str(review_img_dom[1]['src']).split('?')[0] if len(review_img_dom) > 1 else 'No Img'

    # 결과물 정리
    result_str = json.dumps(review_data, ensure_ascii=False)
